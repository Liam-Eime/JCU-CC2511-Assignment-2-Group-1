...
void moving_steps(int *x_move, int *y_move, int *z_move, int *x_place, int *y_place, int *z_place)
{
  int x_time = 0;
  int x_timer = 0;
  int y_time = 0;
...
void draw_circle(int size, int x_place, int y_place, int z_place)
{
  int x_move = 0;
  int y_move = 0;
  int temp_x = x_place;
  ...
  for (int i = 0; i <= (4 * size); i++)
  {
    ...
    moving_steps(&x_move, &y_move, &z_move, &x_place, &y_place, &z_place);
  }
}
      ...
      if ((ch == '\r') || (ch == '\n') || (indexx == 99)) // runs command.
      {
        ...
        else if (sscanf(buffer, "circle %d", &val) > 0) // draws circle radius val.
        {
          ...
          if (true)
          {
            ...
            term_move_to(2, 9);
            term_erase_line();
            printf("Circle R=%d", val);
            term_move_to(2, 10);
            printf("Center (%d, %d)", x_place + val, y_place);
            draw_circle(val, x_place, y_place, z_place);
          }
        }
