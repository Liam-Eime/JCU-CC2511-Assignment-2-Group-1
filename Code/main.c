...
int one_step_in_dir(int move, int step, int dir) // When using this one need to set both dir and step to false after a sleep command
{
  if (move > 0)
  {
    gpio_put(step, true);
...
int debug_step(step, dir) // When using this one no need to set any sleep command
{
...
// Main step code, from having a set move of x y and z it will straightline the dzired amount.
#define moving_steps()                                             \
  {                                                                \
...
int main(void)
{
  ...
  {
    xlist[i / 3] = list[i] * 100;
    ylist[i / 3] = list[i + 1] * 100;
    zlist[i / 3] = list[i + 2] * 100;
  }
  ...
  while (true)
  {
    ...
    while (codeType == 1)//debug using physical buttons.
    {
      ...
      while (debug == 1)
      {
        ...
        // TODO add fall out of debug/settings
        if (debug_target == 0)
        {
    ...
    while (codeType == 3)//Terminal
    {
      ...
      if ((ch == '\r') || (ch == '\n') || (indexx == 99))
      {
        ...
        int volatile valz = 0;
        int volatile val = 0;
        if (sscanf(buffer, "goto %d %d %d", &valx, &valy, &valz) > 0)
        {
          x_move = valx - x_place;
          y_move = valy - y_place;
          z_move = valz - z_place;
        ...
        else if (sscanf(buffer, "move %d %d %d", &x_move, &y_move, &z_move) > 0)
        {
          term_move_to(3, 10);
          term_erase_line();
          term_move_to(3, 9);
        ...
        else if (sscanf(buffer, "codetype %d", &val) > 0)
        {
          if ((val >= 0) && (val <= 4))
          {
            term_cls();
